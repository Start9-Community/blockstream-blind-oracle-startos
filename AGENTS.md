...
- **Package id is `blockstream-blind-oracle`.** It wraps Blockstream's [`blind_pin_server`](https://github.com/Blockstream/blind_pin_server) — the blind PIN oracle a Jade hardware wallet consults when unlocking with a PIN. One HTTP API on port 8096, one `main` volume, no dependencies.
